...
SIMBLLIB_API BYTE Send_RQ(LPBL_Request pRQ)
{
	...
	LPBYTE RQ_Buffer = (LPBYTE) malloc(300);
        ...
        case WRITE_CMD                 : //Write maximum 256 bytes to the RAM or the NVM starting from an address specified by the user
        								{
											...
											memcpy(RQ_Buffer,pRQ->_data,pRQ->_length);
											// Send data
											...
											{
											  // send checksum
											  RQ_Buffer[pRQ->_length] = checksum;

											  if (Cur_COM.sendData(pRQ->_length+1, RQ_Buffer) != pRQ->_length+1)
		                                         return SEND_FAIL;
											}
											else
											{
