...
void man()
{
	printf("STMicroelectronics UART Flash Loader command line v2.8.0 \n\n");
	printf(" Usage : \n\n");
	...
	printf("  -o              (Get or Set STM32F1xx option bytes ONLY : use -d command for other MCUs) \n");              
...
int main(int argc, char* argv[])
{
	...
	char*  filename;
	char   devname[256] = "STM32F1_Low-density_32K.STmap";
	bool   Verify = FALSE;
	bool   optimize = FALSE;
	int    becho = 0;
	...
	else
	{
		...
		while(arg_index < argc)
		{
			...
			else if (strcmp(argv[arg_index],"-c")==0)
			{
			   ...
			   BL_SetEcho(becho);  // Setting Echo back mode
			   ...
			   while(arg_index < argc)
			   {
				 ...
				 else if(Is_SubOption(argv[arg_index]))
				 {
					 ...
					 else 
					 {
						 ...
						 printf("\n Press any key to continue ...\n"); 
						 getchar(); 
						 return 1;   /* return with parameter error */
					 }
				 }
				 else 
				 {
			   ...
			   while(arg_index < argc)
			   {
				 ...
				 else if(Is_SubOption(argv[arg_index]))
				 {
					 if (arg_index< argc) 
					   ...
					   Commands pCmds;
					   CString m_Version;
					   if (BL_GET(&Version, &pCmds) == SUCCESS)
					   {
										...
										if (FILES_GetImageNbElement(Image, &NbElements) == FILES_NOERROR)
										{
											for (int el=0; el< (int)NbElements;el++)
											{
												...
												{	   
													...
													if (FILES_GetImageElement(Image, el, &Element) == FILES_NOERROR)
													{
														if (BL_DNLOAD(Element.dwAddress, Element.Data, Element.dwDataLength, FALSE) != SUCCESS)
														{
											...
											for (el=0; el< (int)NbElements;el++)
											{
												...
												if (FILES_GetImageElement(Image, el, &Element) == FILES_NOERROR)
												{
													...
													if (FILES_GetImageElement(Image, el, &Element) == FILES_NOERROR)
													{
														if (BL_VERIFY(Element.dwAddress, Element.Data, Element.dwDataLength, FALSE) != SUCCESS)
														{
															VerifySuccess = FALSE;
															char str[255];
					 ...
					 else if (strcmp(argv[arg_index-1],"--sec")==0) 
					 {
						...
						int i;
						 ...
						 Res = BL_ERASE(nsec, (LPBYTE)sectors+2);
						 if (Res != SUCCESS)  
						 {
				...
				{   
					...
					{     // STM32 or STR911 is used
						...
						if (BL_GET_ID(&size,NULL) == SUCCESS)
						{
							...
							if (BL_GET_ID(&size,pID) == SUCCESS) // Get the ID
							{
				   ...
				   for (int el=0; el< (int)NbElements;el++)
				   {
						...
						{	   
						   ...
						   if (FILES_GetImageElement(Handle, el, &Element) == FILES_NOERROR)
						   {
							  ...
							  if (BL_DNLOAD(Element.dwAddress, Element.Data, Element.dwDataLength, optimize) != SUCCESS)
							  {
					...
					for (int el=0; el< (int)NbElements;el++)
					{
					    ...
					    if (FILES_GetImageElement(Handle, el, &Element) == FILES_NOERROR)
						{
							...
							if (FILES_GetImageElement(Handle, el, &Element) == FILES_NOERROR)
							{
								...
								if (BL_VERIFY(Element.dwAddress, Element.Data, Element.dwDataLength, optimize) != SUCCESS)
								{
									VerifySuccess = false;
				...
				while(arg_index < argc)
				{
				 ...
				 else if(Is_SubOption(argv[arg_index]))
				 {
					 ...
					 if (strcmp(argv[arg_index-1],"--get")==0) 
					 {
						 ...
						 if(TARGET_GetOptionBytes(ADDR_XX_OPB, &User, &RDP, &Data0, &Data1, &WRP0, &WRP1, &WRP2, &WRP3) == SUCCESS)
						 {
						   ...
						   {
								IMAGEELEMENT Element={0};
								Element.dwAddress   = ADDR_XX_OPB;
								Element.dwDataLength = 16;
								Element.Data = (LPBYTE)malloc(Element.dwDataLength);

								
								
								if ((ADDR_XX_OPB == ADDR_F1_OPB) || ADDR_XX_OPB == ADDR_F0_OPB )
								{
									Element.Data[0]  = RDP;
									Element.Data[1]  = ~RDP;
									Element.Data[2]  = User;
								...
								else	if (ADDR_XX_OPB == ADDR_L1_OPB )
								{
									Element.Data[0]  = RDP;
									Element.Data[1]  = 0x00;
									Element.Data[2]  = ~RDP;
									...
									Element.Data[8]  = WRP0;
									Element.Data[9]  = WRP1;
									Element.Data[10] = ~WRP0;
									Element.Data[11] = ~WRP1;
									Element.Data[12] = WRP2;
									Element.Data[13] = WRP3;
									Element.Data[14] = ~WRP2;
									Element.Data[15] = ~WRP3;
								}
								else	if (ADDR_XX_OPB == ADDR_F2_OPB )
...

								{
									...
									Element.Data[8]  = WRP0;
									Element.Data[9]  = WRP1;
									Element.Data[10] = 0xFF;
									Element.Data[11] = 0xFF;
									Element.Data[12] = 0xFF;
									Element.Data[13] = 0xFF;
									Element.Data[14] = 0xFF;
									Element.Data[15] = 0xFF;
								}
					 ...
					 else if (strcmp(argv[arg_index-1],"--set")==0) 
					 {
									   ...
									   if ( NbElements == 1 )
									   {
										  ...
										  if (FILES_GetImageElement(OPBImage, 0, &Element) == FILES_NOERROR)
										  {
											  ...
											  if (FILES_GetImageElement(OPBImage, 0, &Element) == FILES_NOERROR)
											  {
												  ...
												  if (TARGET_SetOptionBytes(ADDR_XX_OPB,User, RDP, Data0, Data1, WRP0, WRP1, WRP2, WRP3) == SUCCESS)
												  {
						 ...
						 {
							   TARGET_GetOptionBytes(ADDR_XX_OPB,&User, &RDP, &Data0, &Data1, &WRP0, &WRP1, &WRP2, &WRP3);
			...
			else if (strcmp(argv[arg_index],"-p")==0)
			{
				while(arg_index < argc)
				{
				 ...
				 else if(Is_SubOption(argv[arg_index]))
				 {
					 ...
					 {
						 if(BL_READOUT_PERM_UNPROTECT() == SUCCESS) 
						 {
					 ...
					 else if (strcmp(argv[arg_index-1],"--ewp")==0) 
					 {
						...
						int i;
						 ...
						 if(BL_WRITE_PROTECT(((LPBYTE)sectors)[0],&((LPBYTE)sectors)[1]) != SUCCESS) 
					 ...
					 {
						 if(BL_WRITE_PERM_UNPROTECT() != SUCCESS) 
