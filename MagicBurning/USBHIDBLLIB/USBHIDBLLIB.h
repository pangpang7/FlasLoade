...
* File Name          : USBHIDBLLIB.h
* Author             : MCD Application Team
* Version            : v2.8.0
* Date               : 01-September-2015
* Description        : Defines the USBHID  boot loader protocol interface
********************************************************************************
...
#ifdef USBHIDBLLIB_EXPORTS
#define USBHIDBLLIB_API __declspec(dllexport)
#else
#define USBHIDBLLIB_API __declspec(dllimport)
#endif
...
const BYTE SET_PRO_CMD				   = 0xA0; //Set config to programmer
const BYTE GET_PRO_CMD				   = 0xA1; //Get config and info from programmer
const BYTE SET_OFFLINE_CMD			   = 0xA2; //Set offline config to programmer
const BYTE SET_PWD					   = 0xB0; //Set password
const BYTE AUTH_PWD					   = 0xB1; //auth password
const BYTE REMOVE_PWD				   = 0xB2; //remove password
const BYTE CHK_PWD_VALID			   = 0xB3; //check if MCU have valid password
...
enum  INTERFACE_TYPE {USBHID, CAN};
...
typedef struct TARGET_DESCRIPTOR
{
	...
	BYTE MAX_AUTH_NUM				; // max auth password failed NUM,if>this NUM, all main flash and info flash will be earsed
	BYTE CUR_AUTH_NUM				;
	BYTE FlashLock[32]				;
	BYTE FlashBusKey[16]			;
	BYTE FlashBusKeyEnable			;
	BYTE ChipID[8]					;
	BYTE ChipVersion				;
...
typedef struct PRO_DESCRIPTOR
{
	BYTE Version                    ;
	BYTE  PRO_PID[2]                ;
	DWORD MCU_RAM_BASE				;
	DWORD MCU_FLASH_BASE			;
	BYTE  MCU_PID[2]                ;
	BYTE  BURNING_TYPE				; // 0.offline 1.online
	BYTE  MCU_COM					; // 0.STC 1.UART
	BYTE  MCU_page_size				; // page_size / 256 
	DWORD flash_offset				;
	DWORD ram_offset				;
	bool  ram_done					;
	bool  flash_done				;
	BYTE  download_type				; // 0.PRO 1. MCU SRAM 2. MCU FLASH
}* LPPRO_DESCRIPTOR;
...
USBHIDBLLIB_API BYTE BL_SET_PRO(LPPRO_DESCRIPTOR pPro,BYTE cmd);
/************************************************************************************/
...
/*
/************************************************************************************/
/*USBHIDBLLIB_API BYTE ESetCOMSettings(EPortNumber numPort, EBaudRate speedInBaud, EDataBits nbBit,
