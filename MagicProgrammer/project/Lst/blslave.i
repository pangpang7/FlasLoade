...
void USBInit(void)
{
	...
	usbcom.tx_pack[0]=0x07; 
}

void USBOutCB(void)
{
	USBCOM tmp;
	PMAToUserBufferCopy(usbcom.rx_pack, (0x104), (64));
	...
	{
		if((usbcom.tolsize!=tmp.tolsize)
			||(tmp.offset!=usbcom.offset+usbcom.cnt)
			||(tmp.offset+tmp.cnt>tmp.tolsize)
			)
		{
			usbcom.cur_rxstats=COM_IDLE;
			return;
...
void USBIn(void)
{
	...
	if(BL_Request.tx_req)
	{
		...
		while(BL_Request.tx_count>=(55))
		{
			...
			tmp.offset+=tmp.cnt;
			UserToPMABufferCopy(usbcom.tx_pack, (0x100), (64));
	     	SetEPTxCount(((u8)1), (64));
	     	SetEPTxValid(((u8)1));
			while(GetEPTxStatus(((u8)1)) != (0x0020));
...
void RQ_init(void)
{
	...
	BL_Config.download_type=DTYPE_PRO;
...
void RQ_CMDProc(void)
{
	...
	if(!(BL_Config.bl_mode==ON_LINE))
	{
		...
		{
			if((BL_Request.cur_cmd!=(0xA1))
				&&(BL_Request.cur_cmd!=(0xA0))
				&&(BL_Request.cur_cmd!=(0xB1))
				&&(BL_Request.cur_cmd!=(0xB2))
				&&(BL_Request.cur_cmd!=(0xB3))
				&&(BL_Request.cur_cmd!=(0x7F))
				)
				BL_Request.cur_cmd=(0x7E);
		}
	}
	...
	switch(BL_Request.cur_cmd)
	{
		...
		case (0xA1): 
		  ...
		  BL_Request.tx_buf[1]=0x10;
			BL_Request.tx_buf[2]=0x09;
		  BL_Request.tx_buf[3]=0x16;
		  BL_Request.tx_buf[4]=((0x8040000-(48*0x800))>>24)&0xff;
			BL_Request.tx_buf[5]=((0x8040000-(48*0x800))>>16)&0xff;
		  BL_Request.tx_buf[6]=((0x8040000-(48*0x800))>>8)&0xff;
		  BL_Request.tx_buf[7]=((0x8040000-(48*0x800))>>0)&0xff;
		  BL_Request.tx_buf[8]=(0x8040000>>24)&0xff;
			BL_Request.tx_buf[9]=(0x8040000>>16)&0xff;
		  BL_Request.tx_buf[10]=(0x8040000>>8)&0xff;
		  BL_Request.tx_buf[11]=(0x8040000>>0)&0xff;
		...
		case (0xB0):
			...
			else if(BL_Request.cur_status==SUB1)
			{
				...
				if(BL_Config.bl_mode==ON_LINE)
				{
					...
					{
						if(!UART_WritePWD(&BL_Request.rx_buf[1]))
							break;
					}
				}
				else
			...
			else if(BL_Request.cur_status==SUB2)
			{
					...
					&&!checksum(&BL_Request.rx_buf[0],cnt+1+3))
				{
					...
					else
					{
						...
						if(BL_Config.download_type==DTYPE_PRO)
						{
							if(((BL_Config.addr>(0x8008000))&&(BL_Config.addr<(((0x8040000-(48*0x800))-(1*0x800))-1)))
								&&((BL_Config.addr+BL_Config.cnt>(0x8008000))&&(BL_Config.addr+BL_Config.cnt<(((0x8040000-(48*0x800))-(1*0x800))-1))))
							{
							...
							else if(((BL_Config.addr>(0x20000000))&&(BL_Config.addr<(0x20008000-1)))
								&&((BL_Config.addr+BL_Config.cnt>(0x20000000))&&(BL_Config.addr+BL_Config.cnt<(0x20008000-1))))	
							{
		...
		case (0x44):
			...
			else if(BL_Request.cur_status==SUB1)
			{
				...
				else if((BL_Request.rx_buf[0]==0xFF) 
					...
					)
				{
					...
					else
					{
						if(BL_Config.download_type==DTYPE_PRO)
						{
							if(FLASH_Earse((0x8008000),(((0x8040000-(48*0x800))-(1*0x800))-1)))
							{
								BL_Request.tx_count=1;
								BL_Request.tx_buf[0]=(u8)ACK;
								SetTXreq();	
...
void RQ_Proc(void)
{
	...
	if((BL_Request.rx_done)&&(!BL_Request.tx_req))
	{
		...
		if(BL_Request.cur_status==IDLE)
		{
			...
			else if((BL_Request.rx_count==1)&&(BL_Request.rx_buf[0]==(0x7F)))
			{
				BL_Request.cur_cmd=BL_Request.rx_buf[0];
				BL_Request.cur_status=CMD;
			}
