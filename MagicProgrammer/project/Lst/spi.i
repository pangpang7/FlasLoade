...
void SPI_TxRx(u8 addr,u16 TxPassword,u16 *RxPassword1)
{
	...
	for(i=0;i<32;i++)
	{
		...
		ArrayIRX[i]=SI_GetBits(((u16)0x4000));
		SO_SetBits(((u16)0x2000));
		SO_ResetBits(((u16)0x2000));
	}
...
void SPIPassword_Write()
{
	u16 RxPassword=0;
	u16 TxPassword=0;
	u16 testPassword1 = 0,testPassword2 = 0;
	u8 i = 0,j = 0,k = 0;
	u8 PasswordRecord[16] = {0};
	...
	SPIPassword_Write_ArrayITX();
	for(;(i < 8)&&(j < 20) ;i = (i + 4),j = (j + 16) )   
	{
		
		SPI_TxRx(0 + i,TxPassword,&RxPassword);			 
		
		SPI_TxRx(1 + i,TxPassword,&RxPassword);			 
		
		SPI_TxRx(2 + i,TxPassword,&RxPassword);			 
		
		SPI_TxRx(3 + i,TxPassword,&RxPassword);			 
	}
